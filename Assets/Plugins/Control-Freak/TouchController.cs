...
#else
	return ((Screen.dpi != 0) ? Screen.dpi : DEFAULT_MONITOR_DPI);
#endif
	}
...
#		endif
	...
	//int touchTotalCount = touchCount;
...
//#endif		
		...
		if ((touchPhase == TouchPhase.Began) && (this.maskAreas != null))
			{
		...
		switch (touchPhase)
			...
			case TouchPhase.Began :
				...
				for (int si = 0; si < MAX_EVENT_SHARE_COUNT; ++si)
					...
					//float 				closestHitDist 	= 0;
					//int					closestHitPrio 	= 0;
					...
					if (closestHit != null)
						...
						if (closestHit.OnTouchStart(touchId, touchPos) == EventResult.SHARED)
			...
			case TouchPhase.Canceled :
				for (int i = 0; i < this.touchables.Count; ++i)
					{
					//if (
					this.touchables[i].OnTouchEnd(touchId);
					 //, touchPos)) break;
					}
				break;
			...
			case TouchPhase.Stationary :
				for (int i = 0; i < this.touchables.Count; ++i)
					{
					//if (
					this.touchables[i].OnTouchMove(touchId, touchPos);
					//) break;
					}
				break;
			}
	...
	// TODO : setup new key masks, joy state and stuff!!
	}
...
public float GetDPI()
	...
	return ((Screen.dpi <= 0) ? this.monitorDpi : Screen.dpi);
	}
