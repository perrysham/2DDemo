...
public class MultiplayerGui : BaseGui {
	...
	protected override void DoGUI() {
		...
		case NetworkManager.GameState.SettingUp:
			ShowSettingUp();
			break;
		case NetworkManager.GameState.SetupFailed:
			ShowError("Race setup failed.");
			break;
		case NetworkManager.GameState.Aborted:
			ShowError("Race aborted.");
			break;
		case NetworkManager.GameState.Finished:
			ShowResult();
			break;
	...
	void ShowSettingUp() {
		GuiLabel(CenterLabelCfg, "Feature coming soon !!!!!");
		
		GuiProgressBar(ProgressBarCfg, ProgressBarFgTexture, ProgressBarBgTexture,
		               NetworkManager.Instance.RoomSetupProgress);
		
		ShowAbortButton("<< Back");
	}
	...
	void Update() {
		if (NetworkManager.Instance != null && NetworkManager.Instance.State == NetworkManager.GameState.Playing) {
			NetworkManager.Instance.UpdateSelf(Time.deltaTime, 0);
	...
	void ShowResult() {
		int rank = NetworkManager.Instance.FinishRank;
